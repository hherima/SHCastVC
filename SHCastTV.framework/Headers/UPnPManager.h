...
static NSString * _Nullable const UPnPVideoStateChangedNotification = @"UPnPVideoStateChangedNotification";   //视频播放状态改变通知
...
- (void)uPnpManager:(UPnPManager * _Nonnull)manager didCloseWithError:(NSError*)err;
...
/**
 请求服务描述文档
 */
- (void)fetchServiceSDD:(Service *)service successHandler:(SDDHandler _Nullable)sddBlk failureHandler:(failureHandler _Nullable)failBlk;
 ...
 */
- (void)setAVTransportURI:(NSString * _Nonnull)uri andTitle:(NSString *_Nonnull)title protocolInfo:(NSString *_Nonnull)protocolInfo response:(ActionResponseHandler _Nonnull)responseHandler;
...
/**
 请求视频连接参数
 */
typedef void (^GetProtocolInfoResponseHandler)(GetProtocolInfoResponse * _Nullable getProtocolInfoResponse, NSURLResponse * _Nullable response, NSError * _Nullable error);
- (void)getProtocolInfo:(GetProtocolInfoResponseHandler _Nullable)responseHandler;
